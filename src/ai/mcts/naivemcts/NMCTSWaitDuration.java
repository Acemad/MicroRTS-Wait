 ...
 * @author santi
 */
public class NMCTSWaitDuration extends AIWithComputationBudget implements InterruptibleAI {
    ...
    protected NMCTSWaitDurationNode tree;
    ...
    public int global_strategy = NMCTSWaitDurationNode.E_GREEDY;
    ...
    public int playoutWaitDuration;

    public NMCTSWaitDuration(UnitTypeTable utt, int waitDuration, int playoutWaitDuration) {
        this(100,-1,100,10,
                0.3f, 0.0f, 0.4f,
                new RandomBiasedAI(),
                new SimpleSqrtEvaluationFunction3(), true, waitDuration, playoutWaitDuration);
    }

    public NMCTSWaitDuration(UnitTypeTable utt) {
        this(100,-1,100,10,
             0.3f, 0.0f, 0.4f,
             new RandomBiasedAI(),
             new SimpleSqrtEvaluationFunction3(), true, 10, 10);
    }


    public NMCTSWaitDuration(int available_time, int max_playouts, int lookahead, int max_depth,
    ...
    public NMCTSWaitDuration(int available_time, int max_playouts, int lookahead, int max_depth,
                             float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef,
                             boolean fensa, int waitDuration, int playoutWaitDuration) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
        ...
        if (playoutWaitDuration != 10)
            this.playoutPolicy = new RandomBiasedWaitDurationAI(playoutWaitDuration);
    }
    ...
    public AI clone() {
        return new NMCTSWaitDuration(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH,
                epsilon_l, epsilon_g, epsilon_0,
                playoutPolicy, ef, forceExplorationOfNonSampledActions, waitDuration, playoutWaitDuration);
    }    
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new NMCTSWaitDurationNode(player, 1-player, gs, null, ef.upperBound(gs),
                current_iteration++, forceExplorationOfNonSampledActions, waitDuration);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        ...
        NMCTSWaitDurationNode leaf = tree.selectLeaf(player, 1-player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG>=1) {
            NMCTSWaitDurationNode best = (NMCTSWaitDurationNode) tree.children.get(idx);
...
//            }
        ...
        for(int i = 0;i<tree.children.size();i++) {
            NMCTSWaitDurationNode child = (NMCTSWaitDurationNode)tree.children.get(i);
            if (DEBUG>=2) {
...
//            if (best == null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
    ...
    public NMCTSWaitDurationNode getTree() {
        return tree;
    }
    ...
    @Override
    public String toString() {
        return getClass().getSimpleName() + "(" + waitDuration + ", " + playoutWaitDuration + ")";
    }
