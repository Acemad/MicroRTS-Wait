...
public class NMCTSRIPRWaitDurationNode {
    ...
    private NMCTSRIPRWaitDurationNode parent; // the node's parent, if any
    ...
    private List<NMCTSRIPRWaitDurationNode> children; // the list of this node's children
    ...
    private HashMap<BigInteger, NMCTSRIPRWaitDurationNode> childrenMap = new LinkedHashMap<>(); // Maps a binInteger to a node
     ...
     * @throws Exception
     */
    public NMCTSRIPRWaitDurationNode(int player, GameState gameState, NMCTSRIPRWaitDurationNode parent, int nodeID,
     ...
     * @throws Exception
     */
    public NMCTSRIPRWaitDurationNode selectLeaf(int player, float epsilon0, float epsilonGlobal, float epsilonLocal, int globalStrategy,
        ...
        if (children.size() > 0 && random.nextFloat() >= epsilon0) {
            ...
            NMCTSRIPRWaitDurationNode selected = null;
     ...
     * @return
     */
    private NMCTSRIPRWaitDurationNode selectFromGlobalMABEpsilonGreedy(float epsilonGlobal) {

        NMCTSRIPRWaitDurationNode best = null;

        if (random.nextFloat() >= epsilonGlobal) { // Exploit : choose the best child.
            for (NMCTSRIPRWaitDurationNode child : children) {
                if (type == 0) { // Max Node
                    if (best == null ||
     ...
     * @return
     */
    private NMCTSRIPRWaitDurationNode selectFromGlobalMABUCB1(float C, double evaluationBound) {
        NMCTSRIPRWaitDurationNode best = null;
        double bestScore = 0;
        for (NMCTSRIPRWaitDurationNode child : children) {
     ...
     * @return
     */
    private NMCTSRIPRWaitDurationNode selectFromLocalMAB(int player, float epsilon0, float epsilonGlobal, float epsilonLocal,
...
//        System.out.println(playerAction);
        ...
        NMCTSRIPRWaitDurationNode oldChild = childrenMap.get(playerActionCode);
        if (oldChild == null) { // If no node with the same playerActionCode exists, create one.
            ...
            NMCTSRIPRWaitDurationNode newChild = new NMCTSRIPRWaitDurationNode(player, newGameState.clone(), this, nodeID, exploreNonSampledActions, waitDuration);
     ...
     * @param child
     */
    public void backpropagate(double evaluation, NMCTSRIPRWaitDurationNode child) {
    ...
    public List<NMCTSRIPRWaitDurationNode> getChildren() {
        return children;
    }
