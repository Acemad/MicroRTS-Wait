...
import aes.nmcts.NMCTSRandomInactivityFilteringFixedNode;
import ai.RandomBiasedAI;
import ai.RandomBiasedWaitDurationAI;
import ai.core.AI;
...
public class NMCTSRIPFWaitDuration extends AIWithComputationBudget implements InterruptibleAI {

    // Members ************************************************************************************
    private GameState initialGameState;
    private EvaluationFunction evaluationFunction;
    private AI playoutPolicy = new RandomBiasedAI();
    private NMCTSRIPFWaitDurationNode tree;
    ...
    int maxAllowedInactions;

    int waitDuration;
    int playoutWaitDuration;

    public NMCTSRIPFWaitDuration(UnitTypeTable unitTypeTable, int maxAllowedInactions, int waitDuration, int playoutWaitDuration) {
        this(100, -1, 100, 10, 0.4f, 0.0f, 0.3f,
                NMCTSRIPFWaitDurationNode.EPSILON_GREEDY,
                new RandomBiasedAI(),
                new SimpleSqrtEvaluationFunction3(), true,
                maxAllowedInactions, waitDuration, playoutWaitDuration);
    }
     ...
     * @param unitTypeTable
     */
    public NMCTSRIPFWaitDuration(UnitTypeTable unitTypeTable) {
        this(100, -1, 100, 10, 0.4f, 0.0f, 0.3f,
                NMCTSRIPFWaitDurationNode.EPSILON_GREEDY,
                new RandomBiasedAI(),
                new SimpleSqrtEvaluationFunction3(), true, 0, 10,10);
    }
     ...
     */
    public NMCTSRIPFWaitDuration(int timeBudget, int iterationsBudget, int simulationTime, int maxDepth,
                                 ...
                                 int maxAllowedInactions, int waitDuration, int playoutWaitDuration) {
        ...
        this.playoutWaitDuration = playoutWaitDuration;
        if (playoutWaitDuration != 10) this.playoutPolicy = new RandomBiasedWaitDurationAI(playoutWaitDuration);
    }
    ...
    public void startNewComputation(int player, GameState gameState) throws Exception {
        ...
        tree = new NMCTSRIPFWaitDurationNode(player, gameState, null, currentIteration++, exploreNonSampledActions, waitDuration);
    ...
    private boolean monteCarloRun(int player) throws Exception {
        ...
        NMCTSRIPFWaitDurationNode selected = tree.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal,
    ...
    private int getMostVisitedActionIndex() {
        ...
        NMCTSRIPFWaitDurationNode mostVisitedChild = null; // The most visited child
        ...
        for (int index = 0; index < tree.getChildren().size(); index++) {
            NMCTSRIPFWaitDurationNode child = tree.getChildren().get(index);
    ...
    private int getHighestEvaluatedActionIndex() {
        ...
        NMCTSRIPFWaitDurationNode highestEvaluatedChild = null; // The highest evaluated child.
    ...
    @Override
    public AI clone() {
        return new NMCTSRIPFWaitDuration(TIME_BUDGET, ITERATIONS_BUDGET, simulationTime, maxDepth,
                         epsilon0, epsilonGlobal, epsilonLocal, globalStrategy, playoutPolicy,
                         evaluationFunction, exploreNonSampledActions, maxAllowedInactions,
                         waitDuration, playoutWaitDuration);
    }

    @Override
    public String toString() {
        return getClass().getSimpleName() + "(" + maxAllowedInactions + ", " + waitDuration + ", " + playoutWaitDuration + ")";
    }
